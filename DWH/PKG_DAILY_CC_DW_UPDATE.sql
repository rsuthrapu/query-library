...
PROCEDURE SP_CCST_DW_MAIN (
    ...
    WHERE TRANS_DATE >= START_DATE AND TRANS_DATE <= END_DATE OR
          LAST_MODIFIED >= START_DATE AND LAST_MODIFIED <= END_DATE;


   CURSOR CUR_CC_DW_CLAIMANT_ADJUSTER IS
    SELECT *
       ...
       END;
        ...
        BEGIN
          -- DBMS_OUTPUT.PUT_LINE('DELETING CLAIMANT ADJUSTER - BEGIN');
            FOR V_DW_CLAIMANT_ADJUSTER IN CUR_CC_DW_CLAIMANT_ADJUSTER LOOP
...
PROCEDURE SP_CCST_DW_CLAIMANT_DETAIL (
    ...
    BUSENEWLOGIC                NUMBER;
      ...
      WHERE TRANS_DATE >= START_DATE AND TRANS_DATE <= END_DATE OR
        ...
        WHERE
            TRANS_DATE >= START_DATE AND TRANS_DATE <= END_DATE OR
            EX_UPDATETIME >= START_DATE AND EX_UPDATETIME <= END_DATE OR
            CL_UPDATETIME >= START_DATE AND CL_UPDATETIME <= END_DATE 
        ORDER BY
            CL_UPDATETIME;
