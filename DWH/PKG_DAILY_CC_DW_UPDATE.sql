        ...
        WITH CLAIM_DATA AS (
           ...
           -- LEFT OUTER JOIN CC_RIAGREEMENT@ECIG_TO_CC_LINK RIA ON RIA.CLAIMID = C.ID AND RIA.RETIRED = 0
            ...
            SELECT 
              ...
              -- , CCT.TRANS_TYPE_PYMT
               , CCT.LOSS_PAID
               , CCT.UNALLOC_EXPENSE_PAID
               , CCT.ALLOC_EXPENSE_PAID
               , CCT.LOSS_RESERVE
               , CCT.UNALLOC_EXPENSE_RESERVE
               , CCT.ALLOC_EXPENSE_RESERVE
               , RIT.CEDED_LOSS_PAID
               , RIT.CEDED_EXPENSE_PAID
               , RIT.CEDED_LOSS_RESERVE
               , RIT.CEDED_EXPENSE_RESERVE
                , CCT.CHECK_NBR
                , CCT.OPERATOR_ID
                , CCT.CLAIM_DRAFT_TYPE
                , CCT.DRAFT_PAID_AMT
                , RIT.CEDED_ULAE_PAID
                , RIT.CEDED_ULAE_RESERVE
...
PROCEDURE SP_UPDATE_CCST_TO_DW_CLAIM (
    ...
    BEGIN
		...
		FROM DW_CLAIM
		WHERE CLAIM_KEY = REC_CLAIM.CLAIM_KEY
        AND CLAIM_KEY = REC_CLAIM.CLAIM_KEY
		AND	DEC_KEY = REC_CLAIM.DEC_KEY
		AND	INSURED_NAME = REC_CLAIM.INSURED_NAME
		AND	AGENCY_NAME = REC_CLAIM.AGENCY_NAME
		AND	WRITING_COMPANY = REC_CLAIM.WRITING_COMPANY
		AND	CLAIM_NBR = REC_CLAIM.CLAIM_NBR
		AND	DATE_OF_LOSS = REC_CLAIM.DATE_OF_LOSS
		AND	CLAIM_STATUS = REC_CLAIM.CLAIM_STATUS
		AND	CLAIM_STATUS_DATE = REC_CLAIM.CLAIM_STATUS_DATE
        AND SOURCE = 'CC';
...
PROCEDURE SP_UPDATE_CCST_TO_DW_CLAIMANT (
    ...
    BEGIN
		...
		WHERE CLAIM_KEY = REC_CLAIMANT.CLAIM_KEY
        AND CLAIMANT_KEY = REC_CLAIMANT.CLAIMANT_KEY 
        AND CLAIM_BRANCH_NBR = REC_CLAIMANT.CLAIM_BRANCH_NBR
		AND	CLAIM_BRANCH_NAME = REC_CLAIMANT.CLAIM_BRANCH_NAME
		AND	DATE_OF_LOSS = REC_CLAIMANT.DATE_OF_LOSS
		AND	CLAIM_STATUS = REC_CLAIMANT.CLAIM_STATUS
        AND CAT_NBR = REC_CLAIMANT.CAT_NBR
		AND	CAT_DESC = REC_CLAIMANT.CAT_DESC
		AND	CITY = REC_CLAIMANT.CITY
		AND	STATE = REC_CLAIMANT.STATE
		AND	COUNTY = REC_CLAIMANT.COUNTY
        AND LAWSUIT_STATUS = REC_CLAIMANT.LAWSUIT_STATUS
        AND SUIT_TYPE = REC_CLAIMANT.SUIT_TYPE			
		AND	CLAIMANT_NAME = REC_CLAIMANT.CLAIMANT_NAME
		AND	STAFF_ADJUSTER = REC_CLAIMANT.STAFF_ADJUSTER
		AND	SUPERVISOR = REC_CLAIMANT.SUPERVISOR
		AND	IND_ADJUSTER = REC_CLAIMANT.IND_ADJUSTER			
		AND	POLICY_KEY = POLICY_KEY
		AND	POLICY_NBR = REC_CLAIMANT.POLICY_NBR
		AND	INSURED_NAME = REC_CLAIMANT.INSURED_NAME
		AND	AGENCY_CODE = REC_CLAIMANT.AGENCY_CODE
		AND	AGENCY_NAME = REC_CLAIMANT.AGENCY_NAME
		AND	AGENCY_STATE = REC_CLAIMANT.AGENCY_STATE
		AND	WRITING_COMPANY = REC_CLAIMANT.WRITING_COMPANY
		AND	POLICY_BRANCH_NBR =REC_CLAIMANT.POLICY_BRANCH_NBR
		AND	POLICY_BRANCH_NAME = REC_CLAIMANT.POLICY_BRANCH_NAME
		AND	WRITING_COMPANY_NBR = REC_CLAIMANT.WRITING_COMPANY_NBR
        AND ATTORNEY = REC_CLAIMANT.ATTORNEY
        AND BUSINESS_NAME = REC_CLAIMANT.BUSINESS_NAME
        AND CLAIM_NBR=REC_CLAIMANT.CLAIM_NBR
        AND SOURCE = 'CC';
