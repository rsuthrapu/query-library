 Migrated policies from ecig to PC
select 
    PP.POLICYNUMBER as "POLICY NUMBER", PR.CODE as "AGENCY CODE"
    , PR.DESCRIPTION as "AGENCY NAME", PP.EDITEFFECTIVEDATE as EFFECTIVE_DATE
    , PP.PRIMARYINSUREDNAME as LEGAL_NAME, P.PRODUCTCODE, PP.TERMNUMBER as TERM_NUMBER
    , CASE  when PPS.name <> 'Bound' 
            then PPS.name 
            when PPS.name = 'Bound' and PP.CANCELLATIONDATE >= TRUNC(PP.EDITEFFECTIVEDATE) and PP.CANCELLATIONDATE < sysdate 
            then 'Canceled' 
            when PPS.name = 'Bound' and TRUNC(sysdate) >= PP.PERIODEND 
            then 'Expired' 
            when PPS.name = 'Bound' and TRUNC(sysdate) < PP.PERIODSTART 
            then 'Scheduled' 
            else 'Inforce' 
            end as STATUS
    , PP.TOTALCOSTRPT as PREMIUM 
  from PCADMIN.PC_POLICY P 
  inner join PCADMIN.PC_POLICYPERIOD PP  on P.id = PP.POLICYID 
  inner join PCADMIN.PC_PRODUCERCODE PR on PR.id = PP.PRODUCERCODEOFRECORDID 
  inner join PCADMIN.PCTL_POLICYPERIODSTATUS PPS on PP.STATUS = PPS.id 
  inner join PCADMIN.PC_JOB J on J.id = PP.JOBID 
  inner join PCADMIN.PCTL_JOB JT on JT.id = J.SUBTYPE 
  inner join PCADMIN.PC_USER U on U.id = PP.CREATEUSERID 
  inner join PCADMIN.PC_POLICYTERM PT on P.id=PT.POLICYID 
  WHERE  PP.MOSTRECENTMODEL=1  
  and PT.MOSTRECENTTERM=1 
  and PT.MIGRATEDFROMECIG_EXT=1 
  ORDER BY PR.CODE,P.PRODUCTCODE, PP.POLICYNUMBER;
...
--LOSSES 
LOSS_HISTORY as (
select LH.POLICYID AS LH_POLICYID, SUM(LH.TOTALINCURRED) AS TOTAL_LOSS_INCURRED 
from PC_LOSSHISTENTRY LH 
inner join BP7SUBMISSIONS BP7S on BP7S.POLICYID=LH.POLICYID GROUP BY LH.POLICYID
),
 
--BPP CONTENTS ONLY
BP7BPPCONTENTS as (
...
 
 
select BP7S.PERIODSTART, BP7S.JOBNUMBER, BP7S.POLICY_STATUS, BP7S.PRODUCTCODE,BP7S.POLICYNUMBER, BP7S.AGENT_SELECTION,
...
, IRPM.PATTERNCODE, IRPM.IRPM_PERCENT
